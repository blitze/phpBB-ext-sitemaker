...
if (empty($editing))
{
	...
	$access_key = sha1($user->data['user_form_salt'] . 'filemanager');

	// We enable super_globals here because RF uses them
	$request->enable_super_globals();

	if (session_id() == '') session_start();
	$_SESSION["RF"]["subfolder"] = $user->data['user_id'];
}
...
$config = array(
	...
	|
	*/
	'base_url' => generate_board_url(), // ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && ! in_array(strtolower($_SERVER['HTTPS']), array( 'off', 'no' ))) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'],
	...
	|
	*/
	'upload_dir' => '/images/sitemaker_uploads/source/',
	/*
	|--------------------------------------------------------------------------
	...
	|
	*/
	'current_path' => $phpbb_root_path . 'images/sitemaker_uploads/source/',
	...
	|
	*/
	'thumbs_base_path' => $phpbb_root_path . 'images/sitemaker_uploads/thumbs/',
