...
class cfg_fields_test extends \phpbb_database_test_case
{
	...
	public function get_edit_form_test_data()
	{
		return array(
			array(
				array(
					'bid'			=> 1,
					'name'			=> 'blitze.sitemaker.block.login',
					'permission'	=> array(),
					'class'			=> '',
					'status'		=> 1,
					'type'			=> 0,
				...
				array(
					...
					'show_member_menu'	=> array(
						'lang'		=> 'SHOW_MEMBER_MENU',
						'validate'	=> 'bool',
						'type'		=> 'radio:yes_no',
						'explain'	=> false,
						'default'	=> 1,
	...
	/**
	 * Test the get_edit_form method
	 *
	 * @dataProvider get_edit_form_test_data
	 */
	public function test_get_edit_form(array $block_data, array $default_settings, $expected)
	{
		$cfg_fields = $this->get_service();
		$html = $cfg_fields->get_edit_form($block_data, $default_settings);

		//$this->assertEquals($expected, $html['options']);
	}
